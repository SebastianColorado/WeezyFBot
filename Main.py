...
def getTrend():
    """Get the first trending topic that starts with f."""
    # Retrieve trending topics from the USA and from the entire world and
    # concatenate the two lists together
    trends = twitterApi.GetTrendsWoeid(23424977) +\
        twitterApi.GetTrendsCurrent()
...
def getWord():
    """Get random word starting with f (or occasionally ph)."""
    # Pick f or ph randomly with a higher probability of picking f
    firstLetter = random.choices(['f', 'ph'], [30, 1])[0]

    # Randomly pick a part of speech to search for
    partOfSpeech = random.choices(["noun", "adjective", "verb-transitive"],
                                  [7000, 2000, 400])[0]

    # Define totals for each combination of first letter and part of
    # speech. Hardcoded to avoid making two api calls for each word
    if firstLetter == 'f':
...
while True:
    ...
    if trendOrWord == 'Trend':
        ...
        lastTrendsTweeted.appendleft(endingPhrase)

    else:
        endingPhrase = getWord()
